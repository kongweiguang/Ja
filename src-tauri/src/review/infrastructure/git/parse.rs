...
/// 解析 porcelain-v2 `-z` 状态记录，不把文件名解释为 shell 语法；分支 header 仅作为
/// 原始 Head 记录保留，因为 Review 的权威分支信息由 catalog 查询独立获取。
pub(crate) fn parse_status(
    ...
    while index < records.len() {
        ...
        match record[0] {
            b'#' => {
                // Review status 只消费文件状态；保留 header 原文可维持 parser 的完整记录预算。
...
/// 暴露解析条目前先为每个非空 porcelain record 计入预算；rename record 同时计入
/// new-path record 与其 NUL companion。
...
/// 严格解码文件名，避免 lossy replacement 将两个路径合并。
