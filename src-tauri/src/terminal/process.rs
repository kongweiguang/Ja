...
impl ProcessTree for UnixProcessTree {
    ...
    fn terminate(&self) -> io::Result<()> {
        ...
        for signal in [SIGTERM, SIGKILL] {
            // 负 pid 精确指向 portable-pty 通过 setsid 创建的 process group。
...
impl WindowsProcessTree {
    ...
    /// 进入 Win32 前先 claim Job handle，保证只有一个 closer 对该进程树调用
    /// `TerminateJobObject` 与 `CloseHandle`。
...
#[cfg(windows)]
/// 原子地把 raw Job handle 转移给唯一 cleanup caller，失败 caller 不再触碰句柄。
pub(crate) fn claim_job_slot(slot: &AtomicPtr<std::ffi::c_void>) -> Option<*mut std::ffi::c_void> {
...
impl ProcessTree for WindowsProcessTree {
    ...
    fn terminate(&self) -> io::Result<()> {
        ...
        if terminate_result.is_ok() || close_result {
            // child 已退出且 TerminateJobObject 与其竞态时，关闭已配置 Job Object 仍是安全
            // finalization；KILL_ON_JOB_CLOSE 标志会继续终止任何残留 descendants。
            Ok(())
        } else {
            terminate_result
        }
