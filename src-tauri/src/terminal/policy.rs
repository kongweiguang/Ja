...
pub(crate) const MIN_TERMINAL_EVENT_BYTES: usize = 64;
...
impl Default for TerminalLimits {
    /// 默认预算保证单个桌面 workspace 可响应，同时限制 PTY worker 数量与 retained output bytes。
    fn default() -> Self {
        Self {
            max_sessions: 8,
...
impl TerminalLimits {
    ...
    pub(crate) fn validate(self) -> Result<Self, TerminalError> {
        let valid = self.max_sessions > 0
            ...
            // 即使普通 output 全部淘汰也必须容纳 Closed/Error/Exited；queue.rs 使用同一固定下限。
...
impl TerminalPolicy {
    ...
    fn resolve_cwd(&self, requested: Option<&Path>) -> Result<PathBuf, TerminalError> {
        ...
        Ok(interactive_shell_directory(canonical))
    }
}
...
/// 只返回当前平台能由同一启动策略实际解析的闭集，避免 UI 展示必然失败的 shell。
pub(crate) fn available_shell_profiles() -> Vec<ShellProfile> {
    #[cfg(windows)]
    let candidates = [
    ...
    let candidates: [ShellProfile; 0] = [];

    candidates
        .into_iter()
        .filter(|profile| resolve_shell(*profile).is_ok())
        .collect()
}
            ...
            match profile {
                ShellProfile::PowerShell => {
                    let program_files = std::env::var_os("ProgramFiles")
                        .unwrap_or_else(|| OsString::from(r"C:\Program Files"));
