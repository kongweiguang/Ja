...
正向样本覆盖 v1 握手、ready token、按 ID 重开登记 Workspace 与按 kind 过滤列表、配置读写/repair、credential
...
都必须失败关闭。
Thread MCP 语料覆盖只读 `thread/mcp/read`、服务来源与状态、68 工具目录、未知工具数省略及
10,000 工具数上限；负例固定拒绝越界工具数和端点等敏感路由字段。
