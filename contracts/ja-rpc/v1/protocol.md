...
`ja-rpc/v1` is the JA-RPC 1.0 JSON-RPC 2.0 JSONL contract between native clients and the Java app-server. All requests use `c:` ids and flow from clients to Java; Java never sends a reverse Tool request. Java owns configuration, credentials, workspace identity, Thread/Turn admission, Tools, MCP sessions, approval state, persisted Tool presentations, frozen Turn change artifacts, and SQLite facts; native clients request one selected artifact file at a time.

共享后台通过 `--ja-transport=tcp` 启动，只监听 `127.0.0.1` 随机端口。Java 在 `<run-dir>/app-server.endpoint.json` 原子发布 `{protocolMajor:1,protocolMinor:0,port,serverInstanceId,runtimeGeneration,token}`；token 是 64 位小写十六进制字符，仅保存在当前用户专属 ACL 或 POSIX 0600 文件中。TCP 首行是独立认证帧 `{"token":"..."}`，最多 256 bytes、两秒超时；认证成功后才接收标准 JA-RPC JSONL，失败直接断开。每个连接完成标准 initialize/initialized；实例身份和 generation 均属于唯一 Java owner。断开连接只释放请求路由和观察，不取消在途 Turn。五分钟无连接时服务端尝试正常关闭；有活动 Turn、待审批/澄清或活动 Plan/Goal 时保持运行并重新计时。

认证客户端须在 ready 后调用 `runtime/context/register`，传完整宿主环境对象及 nullable Shell 可执行文件路径；Java 仅在内存中持有该快照，并在 Turn 准入时冻结给派生任务。未注册上下文的业务请求返回 `NOT_INITIALIZED`。`runtime/shutdown {force?:boolean}` 的默认 false 在仍有待处理工作时返回 `INVALID_STATE`；成功结果仍为 `{accepted:true,status:"shutting_down"}`。显式 force 才进入现有有界取消关闭。

`turn/start`、`turn/continue`、`turn/reask`、`approval/respond` 必须携带 `clientOperationId`（`op_` 加 32 位小写十六进制字符）。服务端用同事务的请求指纹和业务回执去重，同 ID 但方法或内容不同返回 `CONFLICT`；`operation/read {clientOperationId}` 返回 `{status:"unknown"}` 或 `{status:"committed",method,threadId,result}`。`threadId` 使客户端重启后仍能定位权威 Thread 快照；unknown 不能证明副作用未发生，断线后客户端保留原输入并等待用户核实，不自动重发。

`thread/observe {threadId}` 在确认前为该连接建立事件观察，`thread/unobserve` 只撤销该连接的观察；两者成功均返回 `{accepted:true,threadId}`。Thread/Turn/Approval 详情事件只发送给观察该 Thread 的连接，runtime/configuration/workspace dirty/thread metadata 保留全局低频广播。Goal 活动、Plan 进度、Task 高频进度还要求当前连接分别持有对应 `goal/observe`、`plan/observe`、`task/observe` 句柄；断开和 unobserve 只释放本连接的句柄，同 Thread 的低频 Goal/Task 发现仍可到达观察者。客户端在 observe ACK 后调用 `thread/read` 对账，期间事件继续排队；连接间通知 `sequence` 沿用进程全局物理写序，过滤其它 Thread 后的缺口正常。`thread/read {threadId,tail:true,limit?,cursor?}` 从最新一页开始，页内 items 仍按旧到新排列，`nextCursor` 指向更旧一页；每页保留权威最新及排队 Turn 身份，`liveStream` 基线由当前 Thread 活动状态决定，与历史分页方向无关。

Initialize requires `protocolMajor=1` and `protocolMinor=0`. Capabilities contain exactly `methods`, `events`, `accessModes`, `collaborationModes`, and `features`. `accessModes` is exactly `["approval_required","full_access"]`, `collaborationModes` is independently `["default","plan"]`, and `features` is `["task_threads_v1","plan_goal_v1","interaction_v1"]`; Plan never expands Tool authority. Limits additionally publish the fixed `maxTurnQueuedInputs=8` and `maxTurnQueuedInputBytes=524288` queue budgets. The Java-owned configuration accepts only `schema_version=1`; every Provider has one required `credential_id`, and that identity is unique across the Provider catalog. The root field `default_access_mode` has only `full_access` and `approval_required`; `thread/create` stores the selected Provider, Model, reasoning effort, access mode, and collaboration mode as durable Thread preferences, while `turn/start` accepts only the Thread identity, strict `content[]`. Approval decisions are only `approve` and `deny`. Each Provider request resolves current preferences, Prompt, Skills and Tool catalog immediately before dispatch; an already dispatched request and an already prepared Tool batch remain stable. `turn/resume` accepts exactly `{turnId,expectedThreadRevision}` and resumes only Java-owned persisted execution state; `turn/cancel` accepts exactly `{turnId}` and is an idempotent highest-priority stop intent whose authoritative Thread revision and Turn mutation version are read inside SQLite; repeated claims reuse the first receipt, and a natural terminal or cancellation race returns the current terminal ACK. Rust and React do not hold a recovery cursor. `turn/input/enqueue`, `turn/input/prioritize`, `turn/input/update`, and `turn/input/delete` are the only pending-input mutation methods.
...
`thread/compact` is the only client-triggered context compaction method. It is guarded by Thread revision CAS and the idle-state invariant, and it does not authorize a normal assistant generation. `thread/compact/cancel` stops only the same connection's in-flight manual request; its ACK is not a completion claim. Automatic, manual, and overflow-recovery compaction share the same Thread-level lifecycle schema and `ja-context-v1` strategy identity.
...
There is no `runtime/configure`, Host Tool RPC, Sandbox RPC, old Tool alias, session grant, old `turn/start.input`, `turn/resume.force`, `turn/steer`, `turn/follow-up`, raw Tool `arguments/content/value`, `workspaceDirty/dirtyReason`, or compatibility enum. Secret material is legal only in `credential/set.secret` and is never echoed.
