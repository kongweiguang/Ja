...
/// 持有受信任启动配置，惰性创建共享后台客户端 bridge；是否存在 Java owner 由连接握手决定。
#[derive(Clone)]
...
impl RuntimeHost {
    ...
    /// 旧窗口退出标记不能阻止连接健康共享后台；mutex 只覆盖构造，不覆盖慢速握手。
    ...
    /// 停止既有客户端连接；未建立连接时没有应影响共享后台的本地 owner。
    ...
    /// bridge 存在时返回其状态，否则只暴露本窗口尚未连接的 Stopped 投影。
