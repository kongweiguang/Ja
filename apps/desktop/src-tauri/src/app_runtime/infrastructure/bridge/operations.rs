...
fn attachment_request_runtime(
    ...
    if let Some(session) = current.client.session_for_cancellation() {
...
/// 三个 preview 方法共享同一已认证连接、退出取消门禁和结构化 RPC error 映射。
fn attachment_preview_request_runtime(
    config: &LaunchConfig,
    runtime: &mut Option<RunningRuntime>,
    operation: impl FnOnce(
        &mut SharedAppServerClient,
        Duration,
    )
    ...
    let _session_cancellation_guard = SessionCancellationGuard::new(exit_control);
    let timeout = operation_timeout(config.request_timeout, exit_control)?;
    let response = operation(&mut current.client, timeout)
        .map_err(|error| RuntimeCommandError::from_process(&error))?;
    let value = frame_to_value(&response).map_err(|_| RuntimeCommandError::unavailable())?;
    if let Some(error) = value.get("error") {
        return Err(command_error_from_rpc(error));
...
pub(super) fn attachment_preview_open_runtime(
    ...
    let value = attachment_preview_request_runtime(
        config,
        runtime,
        |client, timeout| client.attachment_preview_open(input, timeout),
        exit_control,
    )?;
...
pub(super) fn attachment_preview_read_runtime(
    ...
    let value = attachment_preview_request_runtime(
        config,
        runtime,
        |client, timeout| client.attachment_preview_read(input, timeout),
        exit_control,
    )?;
...
pub(super) fn attachment_preview_close_runtime(
    ...
    let value = attachment_preview_request_runtime(
        config,
        runtime,
        |client, timeout| client.attachment_preview_close(input, timeout),
        exit_control,
    )?;
...
pub(super) fn history_request_runtime(
    ...
    history_frame_result(method, response)
}

/// Actor 和窄压缩 lease 共用严格响应/错误解析，不能因长等待建立第二套 wire 解释。
pub(super) fn history_frame_result(
    method: HistoryMethod,
    response: RpcFrame,
) -> Result<Value, RuntimeCommandError> {
...
/// 当前 Ready generation 只签发固定压缩 lease；模型等待移到 Tauri command 调用线程。
pub(super) fn thread_compaction_lease_runtime(
    runtime: &mut Option<RunningRuntime>,
) -> Result<ThreadCompactionLease, RuntimeCommandError> {
    runtime
        .as_mut()
        .ok_or_else(RuntimeCommandError::unavailable)?
        .client
        .thread_compaction_lease()
        .map_err(|error| RuntimeCommandError::from_process(&error))
}
...
pub(super) fn turn_input_runtime(
    ...
    let requested_turn_id = params
        ...
        .ok_or_else(RuntimeCommandError::invalid_params)?
        .to_owned();
    let operation_id = if method == "turn/input/enqueue" {
        params
    ...
    let _session_cancellation_guard = SessionCancellationGuard::new(exit_control);
    let timeout = operation_timeout(config.request_timeout, exit_control)?;
    let response = match current.client.request(method, params, timeout) {
        Ok(response) => response,
        Err(error) => {
            if let Some(operation_id) = operation_id.as_deref() {
                return recover_enqueued_input(current, operation_id, &requested_turn_id, timeout);
            }
...
/// 响应丢失后只查询同一操作身份与权威队列，不重发可能已接纳的 steering/follow-up。
fn recover_enqueued_input(
    current: &mut RunningRuntime,
    operation_id: &str,
    turn_id: &str,
    timeout: std::time::Duration,
) -> Result<TurnInputResult, RuntimeCommandError> {
    let unknown = || RuntimeCommandError {
        code: "OPERATION_UNCONFIRMED",
        message: "input outcome is unknown; refresh the conversation before submitting again",
        retryable: false,
    };
    let response = current
        .client
        ...
        .map_err(|_| unknown())?;
    let value = frame_to_value(&response).map_err(|_| unknown())?;
    let (thread_id, input_id) =
        input_operation_receipt_identity(&value, turn_id).ok_or_else(unknown)?;
    let thread = current
        .client
        .request(
            "thread/read",
            json!({"threadId":thread_id,"tail":true,"limit":1}),
            timeout,
        )
        .map_err(|_| unknown())?;
    let snapshot = frame_to_value(&thread).map_err(|_| unknown())?;
    let input_queue = parse_input_queue(snapshot.pointer("/result/inputQueue"), turn_id)
        .map_err(|_| unknown())?;
    Ok(TurnInputResult {
        accepted: true,
...
/// 响应丢失后的查询必须同时匹配方法、Turn 和两级身份，不能用相邻请求的回执确认当前输入。
pub(crate) fn input_operation_receipt_identity<'a>(
    value: &'a Value,
    turn_id: &str,
) -> Option<(&'a str, &'a str)> {
    let receipt = value.get("result")?;
    if receipt.get("status")?.as_str()? != "committed"
        || receipt.get("method")?.as_str()? != "turn/input/enqueue"
        || receipt.pointer("/result/turnId")?.as_str()? != turn_id
        || !receipt.pointer("/result/accepted")?.as_bool()?
        || !matches!(
            receipt.pointer("/result/kind")?.as_str()?,
            "steering" | "follow_up"
        )
    {
        return None;
    }
    let thread_id = receipt.get("threadId")?.as_str()?.trim();
    let input_id = receipt.pointer("/result/inputId")?.as_str()?.trim();
    (thread_id.starts_with("thr_")
        && valid_id(thread_id, 128)
        && input_id.starts_with("input_")
        && valid_id(input_id, 128))
    .then_some((thread_id, input_id))
}
