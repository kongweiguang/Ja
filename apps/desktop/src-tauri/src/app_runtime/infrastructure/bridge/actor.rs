...
pub(crate) enum BridgeCommand {
    ...
    ThreadCompactionLease {
        reply: Reply<ThreadCompactionLease>,
    },
    ToolArtifactRead {
...
impl RuntimeBridgeInner {
    /// 设计原因：该函数位于单 owner actor 边界，必须保持命令顺序、终态可见性与有界等待。
    /// 最后一个窗口 owner 消失前发送高优先级本地 shutdown 并 join actor，
    /// 只回收连接与事件 worker，不终止 Java 共享后台。
    ...
    /// 设计原因：该函数位于单 owner actor 边界，必须保持命令顺序、终态可见性与有界等待。
    /// 报告 Tauri 是否可接受最终 Exit，前提是 actor、event worker 和本地连接都已清空。
    ...
    /// 设计原因：该函数位于单 owner actor 边界，必须保持命令顺序、终态可见性与有界等待。
    /// 共享后台不属于窗口进程；桌面退出失败只记录本地连接诊断，不能阻断其他客户端恢复。
    pub(super) fn record_forced_exit(&self) {
        tracing::error!(
            generation = self.cleanup_fault.generation(),
            "desktop connection cleanup was unconfirmed before exit"
        );
    }
}
...
/// 设计原因：该函数位于单 owner actor 边界，必须保持命令顺序、终态可见性与有界等待。
/// 串行执行共享客户端操作，同时独立轮询 EventPump，避免请求阻塞事件消费。
pub(super) fn actor_loop(
    ...
    let mut pending_cleanup: Option<SharedAppServerClient> = None;
    ...
    loop {
        ...
        match command_receiver.recv_timeout(ACTOR_POLL_TIMEOUT) {
            ...
            Ok(BridgeCommand::ThreadCompactionLease { reply }) => {
                let _ = reply.send(thread_compaction_lease_runtime(&mut runtime));
            }
    ...
    if !shutdown_completed {
        ...
        if !cleanup_confirmed(
            ...
            if let Some(client) = pending_cleanup.take()
                && let Err(error) = context
                    .quarantine
                    .retain_pending(client, &context.cleanup_fault)
            {
