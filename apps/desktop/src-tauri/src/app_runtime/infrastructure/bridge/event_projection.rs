    ...
    let join = thread::Builder::new()
        ...
        .spawn(move || {
            event_drain_loop(EventDrainContext {
                ...
                client_generation: spec.client_generation,
    ...
    loop {
        ...
        match event {
            ...
            SessionEvent::ProcessExited {
                generation: observed,
                code,
            } if observed == context.client_generation => {
                context
                    .terminal_fault
...
pub(super) fn terminal_runtime(context: TerminalCleanupContext<'_>, reason: &str) {
    ...
    eprintln!("Ja runtime connection terminated: reason={reason} generation={generation}");
...
pub(super) fn snapshot(
    ...
    let debt_generation = cleanup_fault
        .generation()
        .max(pending_cleanup.map_or(0, SharedAppServerClient::generation));
...
pub(super) fn current_status(runtime: &mut RunningRuntime) -> RuntimeStatus {
    RuntimeStatus {
        status: RuntimeStatusKind::from_lifecycle(runtime.client.state()),
