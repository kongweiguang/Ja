...
use super::{EXIT_DEADLINE, LaunchConfig, RuntimeCommandError, SharedAppServerClient};
use std::sync::Arc;
...
/// 将所有测试同步和共享客户端连接变化收敛为一个私有端口。
///
/// 端口不能替换 RPC 方法、响应或领域状态，只能控制 deadline、连接 cleanup 故障与
...
pub(crate) trait RuntimeControlPort: Send + Sync {
    ...
    /// 连接或按需启动真实共享后台；Harness 可在实现内部注入实例级 cleanup 故障。
    fn connect_client(
        &self,
        config: &LaunchConfig,
        host_generation: u64,
    ) -> Result<SharedAppServerClient, RuntimeCommandError> {
        SharedAppServerClient::connect_or_start(config.sidecar.clone(), host_generation)
            .map_err(|error| RuntimeCommandError::from_process(&error))
    }

    /// 本地连接清理必须经过同一个内部端口，测试才能注入一次性失败并验证 quarantine；
    /// 生产实现仍直接调用 `ja-runtime` 的有界 disconnect，不向前端开放后台停止能力。
