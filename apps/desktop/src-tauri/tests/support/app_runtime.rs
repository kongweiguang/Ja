...
use ja_runtime::app_server_process::{SharedAppServerClient, SidecarConfig};
...
impl RuntimeControlPort for HarnessRuntimeControl {
    ...
    /// 在真实连接清理前消费一次故障配额；重试仍经生产断连路径，测试不能杀共享后台。
    fn disconnect_client_until(
        &self,
...
impl RuntimeHostHarness {
    ...
    /// 断线恢复测试从真实 Java 读取权威状态，避免靠旧客户端内存推断后台任务是否存活。
    pub fn read_thread(&self, thread_id: &str) -> Result<Value, RuntimeCommandError> {
        request_history(
            &self.host,
    ...
    /// 固定手动压缩入口仅供隔离 Host 集成验收；结果仍由生产 History DTO 路径校验。
    ...
    /// 与压缩使用同一真实 Host 连接；空闲返回 false，不伪造已经停止的事实。
    pub fn cancel_compaction(&self, thread_id: &str) -> Result<Value, RuntimeCommandError> {
        request_history(
            &self.host,
    ...
    /// 真实断线测试先拿订阅 ACK 再读基线，和桌面 controller 共用同一时序合同。
    pub fn observe_thread(&self, thread_id: &str) -> Result<(), RuntimeCommandError> {
        let response = request_history(
            &self.host,
            HistoryMethod::ThreadObserve,
            serde_json::json!({ "threadId": thread_id }),
        )?;
        if response["accepted"] != true || response["threadId"] != thread_id {
