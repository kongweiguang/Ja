...
public final class StreamContext {
    ...
    /**
     * 将 sink、取消 Controller 和 Tool Catalog 固定到单次请求；重试由会话层另建请求负责。
     */
    public StreamContext(ModelEventSink sink,
    ...
    /**
     * 读取更多 Provider 字节前等待 sink 持久接受事件，失败时停止当前请求。
     */
    public void emit(ModelPort.ModelEvent event) {
        CompletableFuture<Void> accepted = controller.beginSink(sink, Objects.requireNonNull(event, "event"));
        try {
            while (true) {
                try {
